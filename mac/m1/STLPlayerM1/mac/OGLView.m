...
- (id)initWithFrame:(NSRect)frameRect pixelFormat:(NSOpenGLPixelFormat *)format {
    ...
    if (self) {
        ...
        // https://stackoverflow.com/a/18493582
...
- (NSSize)getDPI {
    ...
    if (win == nil)
        return sz;  // best guess for a modern mac
    
    sz.width = sz.height = 0;
    NSDictionary *winDescription = [win deviceDescription];
    assert(winDescription != nil);
    NSValue *szContainer = winDescription[NSDeviceResolution];
    assert(szContainer != nil);
    [szContainer getValue:&sz];
...
- (int)getDPIwidth {
    return [self getDPI].width;
}

- (int)getDPIheight {
    return [self getDPI].height;
}
...
- (void)draw {
    ...
    CGLError errCode = -1;
    errCode = CGLSetCurrentContext([[self openGLContext] CGLContextObj]);
    [self checkCGLErrCode:&errCode];
    
    const int dpiWidth = [self getDPIwidth];
    const int dpiHeight = [self getDPIheight];
    const int resWidth = NSWidth([self frame]) * dpiWidth / 72;
    const int resHeight = NSHeight([self frame]) * dpiHeight / 72;
    assert(resWidth > 1 && resHeight > 1);
